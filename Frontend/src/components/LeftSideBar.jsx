...
function LeftSidebar(props) {
  ...
  function setRating(target) {
    const rating_value = target.getAttribute("value");
    ratingsArray.map((number) => {
      const star = document.querySelector(`#rating_star_${number}`);
      star.setAttribute("fill", "none");
    });

    for (let index = 1; index <= rating_value; index++) {
      const star = document.querySelector(`#rating_star_${index}`);
      star.setAttribute("fill", "yellow");
    }

    props.setRating(target.getAttribute("value"));
  }
    ...
    <>
      <div className="flex flex-col gap-4 p-8">
        <input
          ...
          id="searchbox"
          placeholder="Search"
          className=" max-w-40 mb-4 bg-slate-200 rounded pl-4 mx-auto"
          onChange={(evt) => {
        ...
        <p
          className={`ml-auto mr-auto border-2 px-2 py-1 rounded-md text-${appColors.fgColor}`}
        >
          {props.maxPrice}
        </p>
          ...
          {ratingsArray.map((rating, index) => (
            <button type="button" key={index}>
              ...
              <svg
                ...
                onClick={(evt) => setRating(evt.currentTarget)}

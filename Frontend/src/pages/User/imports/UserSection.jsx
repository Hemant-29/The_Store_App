...
const UserSection = (props) => {
  ...
  const sections = [
    ...
    { key: "userCart", label: "Your Cart" },
    { key: "userFavorites", label: "Your Favorites" },
    { key: "userOrders", label: "Your Orders" },
    { key: "userReviews", label: "Your Reviews" },
  ];
  ...
  const fetchCartProducts = async () => {
    try {
      ...
      // Fetch complete product details for each item in the cart
      const cartProducts = await Promise.all(
        cartItems.map(async (item) => {
          const productResponse = await axios.get(
            `${baseUrl}/api/v1/products/single/${item.productId}`,
            { withCredentials: true }
          );
          return { ...productResponse.data, amount: item.amount }; // Include the amount in the product object
        })
      );
      // console.log("cart Products:", cartProducts);

      setProducts(cartProducts); // Set the fetched product details with amounts
        ...
        return (
          <>
            <TitleBox
              title="User Cart"
              onEditClick={() =>
                (window.locatiproductObject.producton.href = "/user/cart")
              }
              link={"/user/cart"}
            ></TitleBox>
                  ...
                  return (
                    <div className="bg-slate-200 p-4 w-full">
                      <ProductsCardCart
                        ...
                        stock={productObject.product.stock}
